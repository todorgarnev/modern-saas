...
export const updateCustomerRecord = async (stripeCustomer: Stripe.Customer) => {
...
export const deleteCustomerRecord = async (stripeCustomer: Stripe.Customer) => {
	const { error } = await supabaseAdmin
		.from("billing_customers")
		.delete()
...
export const getCustomerRecord = async (user_id: string) => {
	...
	const stripeCustomer = await stripe.customers.create({
		...
		// test_clock: testClock.id // STRIPE TEST CLOCK
	});
